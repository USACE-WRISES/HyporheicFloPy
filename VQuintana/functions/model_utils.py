...
def build_gwf_model(
    ...
    #     #  modelgrid has real‑world coordinates once DIS is instantiated
    #     mg  = gwf.modelgrid
    #     top = cfg.tops[0]                         # (nrow,ncol) NumPy array

    #     fig, ax = plt.subplots(figsize=(8, 7))
    #     # 1) coloured top layer
    #     im = ax.imshow(
    #         top,
    #         cmap="terrain",
    #         origin="lower",
    ...
    #     cbar.set_label("Top‑layer elevation (ft)")

    #     # 2) grid
    #     mv = flopy.plot.PlotMapView(model=gwf, ax=ax)
    #     mv.plot_grid(linewidth=0.25, color="black", alpha=1.0, zorder=2)

    #     ax.set_title("Top layer & DIS grid (QA preview)")
    #     ax.set_xlabel("Easting");  ax.set_ylabel("Northing")
    #     ax.set_aspect("equal")
    #     plt.tight_layout()
    #     plt.show()
    # except Exception as err:          # never bomb the model build on a QA plot
    #     print(f"[WARN] QA plot failed: {err}")
...
# ---------------------------------------------------------------------
#  Plot the groundwater-flow solution (heads) on several pages
# ---------------------------------------------------------------------
def plot_gwf_all(
    gwfsim: flopy.mf6.MFSimulation,
    *,
    cmap_head: str = "viridis",
) -> None:
    """
    Visual QC plots for a finished MODFLOW 6 run.

    Parameters
    ----------
    gwfsim : flopy.mf6.MFSimulation
        The *completed* simulation (so the head file exists).
    cmap_head : str, default "viridis"
        Matplotlib colormap to use for the head-contour overlays.
    """
    import matplotlib.pyplot as plt
    from matplotlib.colors import Normalize
    from functions import report_utils as reu               # ← local import

    # ── grab the flow model & results ─────────────────────────────────
    gwf   = gwfsim.get_model(gwfsim.model_names[0])
    dis   = gwf.get_package("DIS")
    nlay, nrow, ncol = dis.nlay.data, dis.nrow.data, dis.ncol.data
    idomain          = dis.idomain.array

    # head = flopy.utils.HeadFile(Path(gwfsim.sim_path) / gwf.name + ".hds").get_data()
    headfile_path = Path(gwfsim.sim_path) / (gwf.name + ".hds")
    head = flopy.utils.HeadFile(headfile_path).get_data()
    surf = dis.top.array

    # pick layers to preview
    layers_qc = [0, nlay // 2, nlay - 1]

    # -----------------------------------------------------------------
    # 1) Heads on a single page (stacked)
    # -----------------------------------------------------------------
    with reu.page(title="Ground-water heads · selected layers") as pg:
        for lay in layers_qc:
            h_masked = np.ma.masked_where(idomain[lay] == 0, head[lay])
            top_mask = np.ma.masked_where(idomain[lay] == 0, surf)

            with reu.fig_stacked(
                pg,
                f"Layer {lay + 1}",
                array_shape=h_masked.shape,
                max_width_inches=7.5,
            ) as ax:
                ...
                # contour overlay using **cmap_head**
                if not np.allclose(h_masked.min(), h_masked.max()):
                    levels = np.linspace(h_masked.min(), h_masked.max(), 12)
                    cs = ax.contour(
                        h_masked,
                        levels=levels,
                        colors=None,          # let the cmap drive colours
                        cmap=cmap_head,
                        linewidths=0.8,
                        origin="lower",
                        extent=[0, ncol, 0, nrow],
    ...
    # -----------------------------------------------------------------
    # 2) 3-D terrain quick-look
    # -----------------------------------------------------------------
    
    top = surf

    # coordinate grids in *model* space
    x = np.arange(ncol)
    y = np.arange(nrow)
    x, y = np.meshgrid(x, y)

    grid = pv.StructuredGrid(x, y, top)
    ...
    png = plotter.screenshot()        # returns a NumPy RGBA image

    with reu.page(title="3-D terrain surface (PyVista)") as pg:
        with reu.fig_stacked(pg, "Model topography", array_shape=top.shape) as ax:
            ax.imshow(png)
            ax.set_axis_off()

    # done – figures are on the report already


# ──────────────────────────────────────────────────────────────────────
# Interactive head viewer  (modal dialog with layer-slider)
# ──────────────────────────────────────────────────────────────────────
def interactive_head_viewer(
    gwfsim: flopy.mf6.MFSimulation,
    *,
    cmap_head: str = "viridis",
    tk_parent: tk.Widget | None = None,     # None ⇒ modal MPL window
    dpi: int = 100,
):
    """
    Layer-by-layer head viewer with a Matplotlib Slider.

    Parameters
    ----------
    gwfsim     Finished MODFLOW-6 simulation (so the *.hds* file exists).
    cmap_head  Colormap used for the contour overlay.
    tk_parent  • None → open a normal Matplotlib window (blocks).  
               • Tk widget → embed the figure and *return* ``(fig, canvas)``.
    """
    import matplotlib
    import matplotlib.pyplot as plt
    from matplotlib.widgets import Slider
    ...
    gwf   = gwfsim.get_model(gwfsim.model_names[0])
    dis   = gwf.dis
    mg    = gwf.modelgrid                     # <- has real-world x/y
    nlay  = dis.nlay.data
    idom  = dis.idomain.array                # (nlay,nrow,ncol) bool/int
    surf  = dis.top.array

    head  = flopy.utils.HeadFile(
        Path(gwfsim.sim_path) / (gwf.name + ".hds")
    ).get_data()

    # ── extents in *coordinate* space ────────────────────────────────
    # mask where *any* layer is active so we zoom to the real domain
    active_mask_2d = idom.any(axis=0)
    rows, cols     = np.where(active_mask_2d)

    # grid-cell centres → coordinates
    xcent = mg.xcellcenters[rows, cols]
    ycent = mg.ycellcenters[rows, cols]

    xmin, xmax = xcent.min(), xcent.max()
    ymin, ymax = ycent.min(), ycent.max()

    # widen a tad so outer cell draws completely
    pad_x = mg.delr[0] * 0.5 if np.allclose(mg.delr, mg.delr[0]) else 0
    pad_y = mg.delc[0] * 0.5 if np.allclose(mg.delc, mg.delc[0]) else 0
    extent = [xmin - pad_x, xmax + pad_x, ymin - pad_y, ymax + pad_y]

    # ── figure / canvas scaffold ────────────────────────────────────
    if tk_parent is None:                    # stand-alone MPL window
        fig, ax = plt.subplots(figsize=(7, 6), dpi=dpi)
        canvas  = fig.canvas
    else:                                    # embed inside Tk widget
        fig = matplotlib.figure.Figure(figsize=(7, 6), dpi=dpi)
        ax  = fig.add_subplot(111)
        canvas = FigureCanvasTkAgg(fig, master=tk_parent)
        canvas.draw()
        canvas.get_tk_widget().pack(fill="both", expand=True)

    plt.subplots_adjust(bottom=0.18)         # room for slider knob

    # ── slider widget ───────────────────────────────────────────────
    s_ax  = fig.add_axes([0.15, 0.05, 0.7, 0.03])
    s_lay = Slider(
        ax      = s_ax,
        label   = "Layer",
        valmin  = 1,
    ...
    fig._slider = s_lay                      # stop GC removing it

    # ── drawing helper ──────────────────────────────────────────────
    def _draw_layer(k: int) -> None:
        ax.clear()

        h_mask = np.ma.masked_where(idom[k] == 0, head[k])
        top    = np.ma.masked_where(idom[k] == 0, surf)
        ...
        if not np.allclose(h_mask.min(), h_mask.max()):
            levels = np.linspace(h_mask.min(), h_mask.max(), 12)
            ax.contour(
                h_mask,
                levels=levels,
                cmap=cmap_head,
        ...
        ax.set_title(f"Ground-water head – layer {k + 1}")
    ...
    # slider callback (lambda keeps signature tiny)
    s_lay.on_changed(lambda v: _draw_layer(int(v) - 1))

    # ── finish ───────────────────────────────────────────────────────
    if tk_parent is None:
        plt.show(block=True)                 # modal window
        return None, None
    else:
        return fig, canvas                   # let caller manage layout
